...
from mapplizer.kml import _cdata, render_kml, write_kmz
...
def test_cdata_terminator_in_content_is_neutralized():
    """Description content is HTML-escaped, so "]]>" cannot close the section."""
    guide = _guide()
    guide.places[0].address_lines = ["evil ]]> injection"]
    root = ET.fromstring(render_kml(guide))
    description = root.find(".//k:Placemark/k:description", NS).text
    assert "evil ]]&gt; injection" in description
